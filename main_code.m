...
for t=T_start:hg:T_final 
    if (mod(t,T_p)==1)         % Check condition for start of the prediction horizon (of T_p seconds)
        ...
        limit_n.ug_vars_high=0.5*limit_n.ug_sps_high; % setting the \Delta ug limits to be
        limit_n.ug_vars_low=-0.5*limit_n.ug_sps_high; % ±50% of \bar u_gmax
...
new_plot_final_result_BTG(T_final,N,Ng,Nb,hg,hb,T_p,z,g_s,b_s,test.baseMVA)
