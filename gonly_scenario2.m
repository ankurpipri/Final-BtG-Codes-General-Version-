...
test=case30(1.3);    % input the desired case, 
% test=case57(1);
...
[N,Ng,Nb,Nc,buildings]=start_for_gonly_scenario2();
...
% two types of optimization problems 1. OPF with grid MPC and 2. only grid MPC as explained in the paper. 
% These matrices will then be fed as inputs to solvers like quadprog or cplex etc.

% This is for decoupled MPC problem simulation, Scenario 2-Optimal building
% inputs are loaded for grid MPC 
load('building_ip');
...
for t=T_start:hg:T_final 
    if (mod(t,T_p)==1)         % Check condition for start of the prediction horizon (of time_predict seconds)
        ...
        % The function 'Problem_1gonly()' returns the dynamic part of the quadprog inputs
        % to Problem type where OPF and grid inputs are optimized 
        ...
        separator_1_gonly(X,N,Nb,Ng,g_s,b_s,count_tg,count_tb,count_tp);  
        % Function to segregate the optimisation variable into final result
        clear X;
        opt_ub=Result.B_HVAC(:,count_tb+1);                        
        % Optimal HVAC Power for Nb buildings to be used as a constant till the next building state change
        ...
        % only the first set of optimal states is taken into the final 
    ...
    elseif (mod(t,hb)==1)         % Check condition for start of the building sampling period
        ...
        % The function 'Problem_3_gonly()' returns the dynamic part of the quadprog inputs
        % to Problem type where only the grid controls are optimized as explained in the paper.
        % Basically this is the RHS of the constraints (disturbances) which keeps on
        % changing as we move ahead in time.
        [X,obj,exitflag] = cplexqp(P3.H,P3.f,[],[],P3.Aeq,...
            P3.Beq,P3.lb,P3.ub,[]);  % calling quadprog to solve Problem 
        ...
        % changing as we move ahead in time.
        [X,obj,exitflag] = cplexqp(P3.H,P3.f,[],[],P3.Aeq,...
            P3.Beq,P3.lb,P3.ub,[]);  % calling cplex to solve Problem 
...
% plot_final_result_gonly_scenario_1(T_final,N,Ng,Nb,hg,hb,T_p,z,g_s,b_s,test.baseMVA);
...
plot_final_result_gonly_scenario_2(T_final,N,Ng,Nb,hg,hb,T_p,z,g_s,b_s,test.baseMVA,zone_temp)
